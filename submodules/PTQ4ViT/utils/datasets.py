...
class LoaderGenerator():
    ...
    def bc_calib_loader(self,num=1024,seed=3):
        if self._bc_calib_set is None:
            np.random.seed(seed)
            inds=np.random.permutation(len(self.train_set))[:num]
            self._bc_calib_set=torch.utils.data.Subset(copy.deepcopy(self.train_set),inds)
            self._bc_calib_set.dataset.transform=self.test_transform
        return torch.utils.data.DataLoader(self._bc_calib_set, batch_size=16, shuffle=False,  **self.train_loader_kwargs)
